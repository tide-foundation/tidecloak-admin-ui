...
export default function DetailSettings() {
  ...
  const handleFileChange = (type: 'background' | 'logo') => (value: File | string,) => {
    if (value instanceof File) {
      if (type === 'background') {
        setBackgroundImage(value);
        setBackgroundPreviewUrl(URL.createObjectURL(value));
      } else if (type === 'logo') {
        setLogo(value);
        setLogoPreviewUrl(URL.createObjectURL(value));
      }
  ...
  const save = async (savedProvider?: IdentityProviderRepresentation) => {
    try {
      ...
      const { LogoURL, ImageURL, backupOn } = provider!.config!;
      ...
      if (providerId === "tide" && tideComponent) {
        ...
        if (resignSettingsRequired) {
          const sigs = await resignSettings(adminClient, realm, getValues(), realmRepresentation!);
          if (sigs === undefined) {
            addAlert(t("Signing settings failed"), AlertVariant.warning)
            return;
          }
          setValue("config.loginURLSig", sigs[1])
          setValue("config.linkTideURLSig", sigs[2])

          // Vendor Settings signaure
          setValue("config.settingsSig", sigs[sigs.length - 2])
          // Vendor Rotating Public
          setValue("config.gVRKSig", sigs[sigs.length - 1])
        }
      }
